...
def get_reddit_data(keyword="healthcare", sort_by="RELEVANCE", max_results=10):
    conn = http.client.HTTPSConnection("reddit-scraper2.p.rapidapi.com")
    
    # Construct the endpoint to search posts
    endpoint = f"/search_posts_v3?query={keyword.replace(' ', '%20')}&sort={sort_by}&time=all&nsfw=0"
    ...
    try:
        ...
        if 'data' in response:
            posts = []
            for post in response['data'][:max_results]:  # Limit to max_results
                # Calculate engagement as sum of interactions
                ...
                # Get subreddit name
                subreddit_name = ""
                if 'subreddit' in post and post['subreddit'] and 'name' in post['subreddit']:
                    subreddit_name = f"r/{post['subreddit']['name']}"
                
                # Get post content
                content = post.get('title', '')
                # Add text content if available
                if 'content' in post and post['content'] and 'text' in post['content'] and post['content']['text']:
                    content += f"\n\n{post['content']['text']}"
                
                # Get image URL if available
                image_url = ""
                if 'content' in post and post['content'] and 'image' in post['content'] and post['content']['image']:
                    image_url = post['content']['image'].get('url', '')
                
                # Convert timestamp to datetime
                created_time = datetime.now()
                if 'creationDate' in post:
                    try:
                        created_time = datetime.strptime(post['creationDate'], "%Y-%m-%dT%H:%M:%S.%f%z")
                    except Exception:
                        pass
                ...
                posts.append({
                    ...
                    "Date": created_time,
                    "Engagement": engagement,
                    "Author": f"{author_name} in {subreddit_name}",
                    "URL": post_url,
                    "ImageURL": image_url
                })
            return pd.DataFrame(posts)
        else:
            st.error("Failed to retrieve Reddit data. API response format unexpected.")
            return pd.DataFrame()
    except Exception as e:
...
if platform == "Reddit":
    ...
    reddit_sort = st.sidebar.selectbox(
        "Sort By",
        options=list(reddit_sort_options.keys()),
        format_func=lambda x: reddit_sort_options[x],
        index=0
    )
...
if platform == "LinkedIn" and linkedin_keyword and linkedin_keyword.lower() != "":
    ...
    with st.spinner("Fetching Twitter data..."):
        try:
            ...
            df = df.sort_values(by="Engagement", ascending=False)
            st.warning("Using mockup data due to API error.")
elif platform == "Reddit" and reddit_keyword and reddit_keyword.lower() != "":
    # Use Reddit API if a keyword is provided for Reddit
    with st.spinner("Fetching Reddit data..."):
        try:
            reddit_df = get_reddit_data(reddit_keyword, reddit_sort, reddit_max_results)
            print(reddit_df)
            if not reddit_df.empty:
                df = reddit_df
                # Sort by engagement in descending order
                df = df.sort_values(by="Engagement", ascending=False)
                st.success(f"Successfully retrieved {len(df)} Reddit posts")
                
                # Automatically save results to Google Sheet
                add_to_google_sheet(df, reddit_keyword)
            else:
                st.warning("Could not retrieve Reddit data. Using mockup data instead.")
                df = load_mock_data_from_sheet()
                # Filter to only Reddit posts
    ...
    for i, row in df.iterrows():
        with st.container():
            ...
            with col2:
                ...
                # Display image if available (for Reddit posts)
                if row["Platform"] == "Reddit" and "ImageURL" in row and row["ImageURL"]:
